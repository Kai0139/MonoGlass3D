...
class GDDTestDataset(Dataset):
    def __init__(self, root_dir, target_size=512):
        super(GDDTestDataset, self).__init__()
...
class GDDDataset(Dataset):
    def __init__(self, root_dir, train=True, target_size=518):
        super(GDDDataset, self).__init__()
        self.root_dir = Path(root_dir)
        self.target_size = target_size
        self.images_dir = self.root_dir.joinpath("image")
        self.masks_dir = self.root_dir.joinpath("mask")
...
class GDDDataModule(lightning.LightningDataModule):
    def __init__(self, root_dir, batch_size=4, target_size=518):
        super(GDDDataModule, self).__init__()
        self.root_dir = Path(root_dir)
        ...
        self.train_dataset = GDDDataset(root_dir=self.train_dir, target_size=target_size)
        self.val_dataset = GDDDataset(root_dir=self.val_dir, target_size=target_size, train=False)
...
if __name__ == "__main__":
    ...
    datamodule = GDDDataModule(root_dir=root_dir, batch_size=1, target_size=518)
